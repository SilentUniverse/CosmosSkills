...
```
   ...
   │  ├─ /tdd -p：无依赖切片按"波"并发（每 issue 一 subagent        │
...
| 并行排空 | `/tdd -p [<feat>]`（长写 `--parallel`） | 无依赖切片按"波"**并发**：每 issue 一 subagent、独立 worktree，绿了再按依赖顺序合并 | 一批**互相独立**的切片，想把墙钟压到单切片量级 |

**典型节奏：** 搜 `^status: ready-for-human` 挑一条手动做；积了一批 ready-for-agent 就 `/tdd <feat>`（或裸 `/tdd`）串行排空；切片互不依赖时 `/tdd -p` 并发排空。一条依赖长链没有并发空间，用串行即可。（`/to-issues` 建完 DAG 会按波宽直接建议串行还是并行。）
...
| [tdd](engineering/tdd/SKILL.md) | 跑红绿循环：`<path>` 单条 · 裸跑串行排空所有 ready · `<feat>` 排空单 feature · `-p`（`--parallel`）无依赖切片并发排空（详见 [DRAIN.md](engineering/tdd/DRAIN.md)） |
