...
Companion to CLAUDE.md §7. The three hard rules live there (directory truth, explicit UTF-8 PowerShell wrapping, PS/cmd never write files) — they must fire at action time, so they stay resident. This file holds the lookup content: when to reach for PowerShell natively, and the notes behind the rules.
...
- **Pipe into PS** — set `[Console]::InputEncoding` to UTF-8 before reading stdin.
...
- When invoking PowerShell from bash, set encoding in that process before producing output:
...
- **Two path worlds** — git-bash's `/tmp` is an MSYS virtual mount; a native process (`python`, `pwsh`, `cmd`, `node`, the modern tools `rg`/`fd`/`sd`/`jq`/`yq`/`ast-grep`, winget tools) resolves `/tmp` as `<cwd-drive>:\tmp` or fails outright. `rg`/`fd`/`sd`/`jq`/`yq`/`ast-grep` look bash-native but are native exes, so a `/tmp/...` path argument to them is as broken as one to `python`. Hand native processes Windows absolute paths; temp files go to `$env:TEMP` (`cygpath -w "$TEMP"` converts from bash), and prefer piping over a temp file. The shell-guardrails path tier blocks POSIX path tokens on native-exe segments.
