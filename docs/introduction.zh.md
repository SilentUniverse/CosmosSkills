...
**一道机器门。** 一个名为 verify-artifacts.py 的验证脚本守在提交之前：完成记录里点名的每一个测试文件，必须真实存在于磁盘上——偷偷删掉测试再汇报全绿，当场红灯。依赖图有环、frontmatter 缺字段、需求变更绕过对账报告，全部拦截。多数工作流的审计停在代码评审层，这道门一直查到证据层。
...
**一条闭环。** spec 拆卡 → tdd 执行 → 双轴审查 + 一屏报告 → tidy 回收。任务卡自足（只看一张卡即可开工），handoff 消费即删，过夜由 overnight.py 逐波换新会话执行——早上面对的每一项人工验证，都附带可直接粘贴的命令。
...
太阳系有九颗行星，这套系统有九条定律，而作者的网名，叫静默宇宙。

九条定律，九颗行星，一个静默宇宙——CosmosSkills。
