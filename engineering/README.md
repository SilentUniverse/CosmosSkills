...
- **[tdd](./tdd/SKILL.md)** — Test-driven development with a red-green-refactor loop. Five ways to invoke: `/tdd <issue-path>` runs one issue; bare `/tdd` drains every ready-for-agent issue serially in dependency order; `/tdd <feat>` drains one feature; `/tdd -p [<feat>]` drains independent waves concurrently (one subagent per issue, own worktree) for the big wall-clock win; a natural-language ask falls back to interview mode.
