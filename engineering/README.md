...
- **[tdd](./tdd/SKILL.md)** — Test-driven development with a red-green-refactor loop. Five ways to invoke: `/tdd <issue-path>` runs one issue; bare `/tdd` drains every ready-for-agent issue serially in dependency order; `/tdd <feat>` drains one feature; `/tdd --parallel [<feat>]` drains independent waves concurrently (one subagent per issue, own worktree) for the big wall-clock win; a natural-language ask falls back to interview mode.
- **[implement](./implement/SKILL.md)** — Thin composition skill: build the agreed slices by driving `/tdd` at pre-agreed seams and closing with `/code-review`. Use when you want to execute a plan, not make one.
- **[route](./route/SKILL.md)** — Router + context-boundary manager. Maps a request onto the next skill (grill → to-prd → to-issues → tdd → code-review → tidy) and decides continue / clear / compact / handoff / subagent so long sessions stay inside the smart zone (fast + sharp).
