...
- **[tdd](./tdd/SKILL.md)** — Test-driven development with a red-green-refactor loop. Five ways to invoke: `/tdd <issue-path>` runs one issue; bare `/tdd` drains every ready issue serially in dependency order; `/tdd <feat>` drains one feature; `/tdd -p [<feat>]` farms ready issues out to subagents (decoupled slices edit the shared tree directly, worktree only when they'd touch the same files) to isolate their output and run independent slices in parallel; a natural-language ask falls back to interview mode.
- **[route](./route/SKILL.md)** — Router + context-boundary manager. Maps a request onto the next skill (grill → plan → tdd → code-review → tidy) and decides continue / clear / compact / handoff / subagent so long sessions stay inside the smart zone (fast + sharp).
- **[plan](./plan/SKILL.md)** — The single planning entry: classifies the ask, grills inline when unclear, supersedes + reconciles on intent change, drafts issues by the dispatchability test, and writes a versioned PRD only when the intent warrants one. (Merged from to-prd + to-issues; their templates and probes live under `plan/` as references.)
- **[atk](./atk/SKILL.md)** — Attack-mode review of the agent's own output (diff, design, plan, decision): fresh-eyes subagent + per-finding verification, first-principles re-derivation, verdict table with deliberate keeps.
