...
### Ways to construct one — start here, escalate as needed
...
When none of these fit the bug, scan the fuller menu — captured-trace replay, throwaway harness,
property/fuzz loop, bisection harness, differential loop, HITL script: **[FEEDBACK-LOOPS.md](FEEDBACK-LOOPS.md)**.
...
**Then ask: what would have prevented this bug?** If the answer involves architectural change (no good test seam, tangled callers, hidden coupling) hand off to the `/improve-arch` skill with the specifics. If the root cause was a `rg`-invisible invariant (hidden constraint, surprising coupling), persist it to the area's `CODEBASE.md` block — two-axis test per `/zoom-out` — and report; no `CODEBASE.md` yet → note it in the wrap-up instead. Make the recommendation **after** the fix is in, not before — you have more information now than when you started.
