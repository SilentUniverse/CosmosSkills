...
Reached from bare `/tdd`, `/tdd <feat>`, or `/tdd --parallel [<feat>]`. Runs a whole
...
1. Enumerate candidates: bare / `--parallel` scans `.scratch/*/issues/*.md` (top level, never
...
## Parallel path (`/tdd --parallel [<feat>]`)
...
Reach for `--parallel` when a feature has several **independent** slices (wide waves). When unsure,
