...
4. After the last issue takes the active set to empty, run the **full suite + build once** as the batch's closing check (§5). Report shipped, failed, deferred, full-suite result, and each shipped issue's `代码讲解`. If a feature's `done` count crossed ~8, suggest `/tidy`.
