...
Loaded on demand by the combined install and verification path in [SKILL.md](SKILL.md), or by a
standalone carrier reference. Policy rules remain with the selected carrier; this file provides the
single-entry JSON shape and combined-engine probes.

## Project (`.claude/settings.json`)

One entry replaces overlapping checks when the requested policy is covered. Preserve an existing
push block; the combined engine allows pushes. Merge settings without replacing unrelated hooks.
Windows invokes an installed `python`; Unix uses `python3` on the same script. A standalone branch
uses the same outer JSON shape with the carrier command named in its reference.
...
Same entry under the `events` key, with `hooks.enabled` true. ZCode runs command hooks sequentially,
so one combined process avoids the latency of two carrier processes. Retained measurements and their
scope live in [README.md](README.md).
...
`tooling/shell-guardrails/`. The runner feeds JSON payloads; it does not execute their commands.
